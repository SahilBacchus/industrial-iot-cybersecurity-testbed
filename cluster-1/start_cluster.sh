...
# Runtime variable for sudo password (leave empty to interactively enter it)
SUDO_PASS=""
...
start_aggregator() {
    ...
    if [ "$duration_min" -eq 0 ]; then
        ...
        ssh -n "${DEV2_USER}@${DEV2_IP}" "mkdir -p ${AGG_DIR} && cd ${AGG_DIR} && (nohup python3 -u UoC_Cluster1_Aggregator.py > aggregator.log 2>&1 &)"
    else
        echo -e "\n---> Starting Aggregator on ${DEV2_USER}@${DEV2_IP} for ${duration_min} minute(s)..."
        ssh -n "${DEV2_USER}@${DEV2_IP}" "mkdir -p ${AGG_DIR} && cd ${AGG_DIR} && (nohup timeout ${duration_min}m python3 -u UoC_Cluster1_Aggregator.py > aggregator.log 2>&1 &)"
    fi
}
...
start_edge() {
    ...
    if [ "$duration_min" -eq 0 ]; then
        ...
        ssh -n "${DEV1_USER}@${DEV1_IP}" "mkdir -p ${EDGE_DIR} && cd ${EDGE_DIR} && (echo '$SUDO_PASS' | sudo -S nohup python3 -u UoC_Cluster1_Edgeserver.py > edgeserver.log 2>&1 &)"
        ssh -n "${DEV1_USER}@${DEV1_IP}" "mkdir -p ${EDGE_DIR} && cd ${EDGE_DIR} && (echo '$SUDO_PASS' | sudo -S nohup python3 -u UoC_Cluster1_Capture_Manager.py > capture.log 2>&1 &)"
    else
        echo -e "\n---> Starting Edge Server components on ${DEV1_USER}@${DEV1_IP} for ${duration_min} minute(s)..."
        ssh -n "${DEV1_USER}@${DEV1_IP}" "mkdir -p ${EDGE_DIR} && cd ${EDGE_DIR} && (echo '$SUDO_PASS' | sudo -S nohup timeout ${duration_min}m python3 -u UoC_Cluster1_Edgeserver.py > edgeserver.log 2>&1 &)"
        ssh -n "${DEV1_USER}@${DEV1_IP}" "mkdir -p ${EDGE_DIR} && cd ${EDGE_DIR} && (echo '$SUDO_PASS' | sudo -S nohup timeout ${duration_min}m python3 -u UoC_Cluster1_Capture_Manager.py > capture.log 2>&1 &)"
    fi
}
